...
print("intializing the sigma run")
ndim = 5 # need to change this for every run!
...
start_theta_total = [1.9, 0.2, 10.6, -0.1, -0.01, 0]
...
datadir = "/Users/jsmonzon/Research/data/paper/model_test/sigma/"
...
def forward(theta):
    models.convert(theta, jsm_SHMR.sigma) # need to change this for every run!
...
test = jsm_mcmc.inspect_run(mcmc_out, data=data, forward=forward, start_theta=start_theta, labels=params, priors=priors, savedir=datadir, min_mass=min_mass, a_stretch=a_stretch)