...
print("intializing the simple run")
ndim = 2 # need to change this for every run!
...
datadir = "/home/jsm99/data/model_test/simple/" # need to change this for every run!
...
def forward(theta):
    models.convert(theta, jsm_SHMR.simple) # need to change this for every run!
...
test = jsm_mcmc.inspect_run(mcmc_out, data=data, forward=forward, start_theta=start_theta,
                            labels=params, priors=priors, savedir=datadir, min_mass=min_mass, a_stretch=a_stretch, SHMR_model="simple")