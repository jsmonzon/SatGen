...
chain_name = "mock_nadler/"
savedir = "../"+chain_name
savefile = savedir+"chain.h5"

data = jsm_models.LOAD_DATA(savedir+"mock_data.npz")
data.get_NADLER_stats(min_mass=config["min_mass"], N_bin=config["N_bin"])
...
def lnlike(theta):
    models.get_NADLER_stats(theta, config["min_mass"],  N_bin=config["N_bin"])
...
def lnprob(theta):
    ...
    return lp + lnlike(theta), lp, lp

dtype = [("prior_D", float), ("prior", float)]
