...
min_mass_overide = config["min_mass"] + 1
print("MINMASS =", min_mass_overide)

data = jsm_models.LOAD_DATA(savedir+"mock_data.npz")
data.get_stats(min_mass=min_mass_overide, max_N=config["max_N"])
...
def lnlike(theta):
    models.get_stats(theta=theta, min_mass=min_mass_overide, max_N=config["max_N"], Nsigma_samples=config["Nsamp"])
