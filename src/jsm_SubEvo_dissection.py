...
datadir="/Users/jsmonzon/Research/StellarHalo/data/local_trees/low_res/test/"
...
def loop(file): 
    ...
    try: 
        name = file[0:-4]+"_evo" 
        #print("evolving", file)
        log_name = file[0:-4]+"_potential_test.txt"  # <<< log file

        # open log file once per tree
        with open(log_name, 'w') as log:
            log.write(f"{'z':>8} {'id':>6} {'order':>6} {'parent':>8} "
                    f"{'M_sub(<rmax)':>15} {'M_host(<rmax)':>15} "
                    f"{'ch_sub':>12} {'ch_host':>12}\n")
            log.write("-" * 65 + "\n")
                ...
                for level in levels: #loop from low-order to high-order systems
                    for id in idx: # loop over branches
                        ...
                        if(iz <= izroot[id]):
                            ...
                            #### this is the test to see if the potentials are updated!!!!!
                            try:
                                log.write(f"{z:>8.3f} {id:>6d} {order[id,iz]:>6d} "
                                        f"{ip:>8d} "
                                        f"{s.M(s.rmax):>15.3e} "
                                        f"{p.M(p.rmax):>15.3e} "
                                        f"{s.ch:>12.3f} "
