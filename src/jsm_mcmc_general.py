...
def RUN(theta_0, lnprob, nwalkers, niter, ndim, ncores, a_stretch):
...
class inspect_run:
    ...
    def chisq_last_sample(self):
        fig, axs = plt.subplots(self.ndim, sharey=True, figsize=(8,8))
        ...
        for i in range(self.ndim):
            ...
            axs[i].set_ylabel("$\\chi^2$", fontsize=12)
    ...
    def SHMR_last_sample(self, color_ind=None):
            ...
            self.fid_Ms = jsm_SHMR.deterministic([self.data.truths[0], self.data.truths[1], self.data.truths[2]], self.halo_masses)
            for i,val in enumerate(self.last_samp):  # now pushing all thetas through!
                SHMR_mat[i] = jsm_SHMR.deterministic([val[0], val[1], val[2]], self.halo_masses)
            ...
            plt.colorbar(cmap, label=clabel)
