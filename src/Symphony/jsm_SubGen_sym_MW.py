...
ncores = 16
samples = np.load("../etc/MWx100.npy")
lgMres = 8
...
def loop(itree): 
    ...
    cfg.M0 = 10.**(samples[itree])
    host_mass = np.round(samples[itree],2)
...
if __name__ == "__main__":
    ...
    pool.map(loop, np.arange(0, samples.shape[0]))#, chunksize=1)
